...
static int guiGameNetCompatUpdRefresh(int modified)
{
    int result;
...
static void guiGameShowNetCompatUpdateResult(int result)
{
    switch (result) {
    ...
    while (!done) {
        ...
        switch (ret) {
            case NETUPD_BTN_START:
                if (guiMsgBox(_l(_STR_CONFIRMATION_SETTINGS_UPDATE), 1, NULL)) {
                    ...
                    if ((ret = ethLoadInitModules()) == 0) {
                        ...
                        UpdateFunction = &guiGameNetCompatUpdRefresh;
                        started = 1;
                    } else {
            ...
            case NETUPD_BTN_CANCEL:
                ...
                break;
            default:
                guiGameShowNetCompatUpdateResult(ret);
                done = 1;
                started = 0;
                UpdateFunction = NULL;
                break;
...
void guiGameShowNetCompatUpdateSingle(int id, item_list_t *support, config_set_t *configSet)
{
    ...
    if (guiMsgBox(_l(_STR_CONFIRMATION_SETTINGS_UPDATE), 1, NULL)) {
        ...
        if ((result = ethLoadInitModules()) == 0) {
            ...
            guiGameShowNetCompatUpdateResult(result);
        } else {
            ethDisplayErrorStatus();
        }
    }
...
static int guiGamePadEmuUpdater(int modified)
{
    ...
    diaSetVisible(diaPadEmuConfig, PADCFG_USBDG_MAC, PadEmuMode & EnablePadEmu);
    diaSetVisible(diaPadEmuConfig, PADCFG_PAD_MAC, PadEmuMode & EnablePadEmu);
    diaSetVisible(diaPadEmuConfig, PADCFG_PAIR, PadEmuMode & EnablePadEmu);

    diaSetVisible(diaPadEmuConfig, PADCFG_USBDG_MAC_STR, PadEmuMode & EnablePadEmu);
    diaSetVisible(diaPadEmuConfig, PADCFG_PAD_MAC_STR, PadEmuMode & EnablePadEmu);
    diaSetVisible(diaPadEmuConfig, PADCFG_PAIR_STR, PadEmuMode & EnablePadEmu);

    diaSetVisible(diaPadEmuConfig, PADCFG_BTINFO, PadEmuMode & EnablePadEmu);
    diaSetVisible(diaPadEmuConfig, PADCFG_PADEMU_WORKAROUND, PadEmuMode & EnablePadEmu);
    diaSetVisible(diaPadEmuConfig, PADCFG_PADEMU_WORKAROUND_STR, PadEmuMode & EnablePadEmu);
...
void guiGameShowModesConfig(item_list_t *support)
{
    ...
    if (support->flags & MODE_FLAG_COMPAT_DMA) {
        ...
        diaSetEnum(diaModesConfig, COMPAT_DMA, dmaModes);
    } else {
    ...
    while (result >= COMPAT_NOEXIT) {
        result = diaExecuteDialog(diaModesConfig, result, 1, NULL);

        compatMode = 0;
        for (i = 0; i < COMPAT_MODE_COUNT; ++i) {
            int mdpart;
            diaGetInt(diaModesConfig, COMPAT_MODE_BASE + i, &mdpart);
            compatMode |= (mdpart ? 1 : 0) << i;
        }

        if (result == COMPAT_LOADFROMDISC) {
            if (sysGetDiscID(hexDiscID) >= 0)
                diaSetString(diaModesConfig, COMPAT_GAMEID, hexDiscID);
            else
                guiMsgBox(_l(_STR_ERROR_LOADING_ID), 0, NULL);
        }

        diaGetInt(diaModesConfig, COMPAT_DMA, &dmaMode);
        diaGetString(diaModesConfig, COMPAT_GAMEID, hexid, sizeof(hexid));
        diaGetString(diaModesConfig, COMPAT_ALTSTARTUP, altStartup, sizeof(altStartup));
    }
}
...
void guiGameSaveConfig(config_set_t *configSet, item_list_t *support)
{
    ...
    diaGetString(diaVMCConfig, COMPAT_GAMEID, hexid, sizeof(hexid));
    if (hexid[0] != '\0')
        configSetStr(configSet, CONFIG_ITEM_DNAS, hexid);

    diaGetString(diaVMCConfig, COMPAT_ALTSTARTUP, altStartup, sizeof(altStartup));
    if (altStartup[0] != '\0')
...
void guiGameLoadConfig(item_list_t *support)
{
    ...
    ConfigSource = -1;
    configGetInt(configSet, CONFIG_ITEM_CONFIGSOURCE, &ConfigSource);
    if (ConfigSource == CONFIG_SOURCE_USER)
        snprintf(configSource, sizeof(configSource), _l(_STR_CUSTOMIZED_SETTINGS));
    else if (ConfigSource == CONFIG_SOURCE_DEFAULT)
    ...
    for (i = 0; i < COMPAT_MODE_COUNT; ++i)
        diaSetInt(diaModesConfig, COMPAT_MODE_BASE + i, (compatMode & (1 << i)) > 0 ? 1 : 0);
    ...
    diaSetString(diaModesConfig, COMPAT_GAMEID, hexid);

    configGetStrCopy(configSet, CONFIG_ITEM_ALTSTARTUP, altStartup, sizeof(altStartup));
    diaSetString(diaModesConfig, COMPAT_ALTSTARTUP, altStartup);
